...
static void disk_submit (const char *plugin_instance,
		const char *type,
		long double read, long double write)
{
	value_t values[2];
	...
	for (;;) {
		...
		if (gid->lg_what == ISPROVIDER) {
			...
			devstat_compute_statistics(gsp, gsq, dt, 
			    ...
			    DSM_NONE);
			disk_submit (pp->lg_name, "gdisk_transfers", ld[1], ld[4]);
			disk_submit (pp->lg_name, "gdisk_mbytes", ld[2], ld[5]);
			disk_submit (pp->lg_name, "gdisk_latency", ld[3], ld[6]);
			submit(pp->lg_name, "gdisk_busy", ld[3]);
		}
		*gsq = *gsp;
	}
